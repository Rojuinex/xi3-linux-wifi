...
echo 'echo "Please unplug the Ethernet cable at this time and connect to a wireless network from the icon in the tray. It should look like an Ethernet port." > instructions-pt3' >> /tmp/worker.sh
echo 'kwrite instructions-pt3 </dev/null &>/dev/null &' >> /tmp/worker.sh
echo 'popd' >> /tmp/worker.sh

chmod 755 /tmp/worker.sh

konsole -noclose -e bash -e /tmp/worker.sh
